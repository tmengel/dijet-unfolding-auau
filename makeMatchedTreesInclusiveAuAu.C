...
{
  return std::hypot(first.eta - second.eta, deltaPhi(first.phi, second.phi));
}
...
// is dR < 3, not dR < 0.3.
std::vector<Match> matchJets(
  const std::vector<Jet>& truth,
...
)
{
  ...
  for (const auto& truthJet : truth)
  {
    ...
    {
      if (used[recoIndex]) continue; // already matched
      if (deltaR(truthJet, reco[recoIndex]) >= maximumDeltaR) continue; // outside threshold
      result.emplace_back(truthJet, reco[recoIndex]);
      used[recoIndex] = true;
      break; // hightst pt priority, so stop after first match
    }
  } // for each truth jet
  return result;
}

const Match* findTruthMatch(
...
void makeMatchedTreesInclusiveAuAu(
  ...
  const std::string input_path = "/home/tmengel/PPG14/rootfiles/v001_20260720/hijing_rho_jet.root",
...
)
{
  ...
  const std::string outputDirectory = output_directory.empty() ? rb.get_tntuple_location() : output_directory;
  ...
  {  
    ...
    {
      std::cerr << "Missing branch " << branch << " in " << input_path << std::endl;
      return;
    }
  }
  ...
  {
    const std::string path = Form( "%s/TNTUPLE_DIJET_SIM_r0%d_jet%d_%s.root", outputDirectory.c_str(), cone_size, sampleIds[sample] , suffix.c_str() );
    ...
    {
      outputs[sample].truthEfficiency[centralityBin] = std::make_unique<TEfficiency>( Form("he_pt_truth_%d", centralityBin), "", 25, 0, 50);
      outputs[sample].dijetEfficiency[centralityBin] = std::make_unique<TEfficiency>( Form("he_pt_dijet_%d", centralityBin), "", 25, 0, 50);
      outputs[sample].response[centralityBin] = std::make_unique<TH2D>(Form("h2_ptt_ptrptt_%d", centralityBin), ";p_{T}^{truth};p_{T}^{reco}/p_{T}^{truth}", 25, 0, 50, 120, 0, 1.2);
      outputs[sample].responseProfile[centralityBin] = std::make_unique<TProfile>(Form("hp_ptt_ptrptt_%d", centralityBin), "", 25, 0, 50, "s");
  ...
  for (Long64_t entry = 0; entry < entries; ++entry)
  {
    ...
    {
      if ( recoPt->at(jet) >= 3.0F 
        && recoE->at(jet) >= 0.0F 
        && recoUnsubE->at(jet) >= 0.0F 
        && std::fabs(recoEta->at(jet)) <= 0.8F
      )
      {
    ...
    {
      outputs[sample].truthEfficiency[centralityBin]->Fill( findTruthMatch(matches, truthJet.id) != nullptr, truthJet.pt);
    }
    ...
    for (const auto& match : matches)
    {
      ...
      {
        outputs[sample].response[centralityBin]->Fill( match.first.pt, match.second.pt/match.first.pt);
        outputs[sample].responseProfile[centralityBin]->Fill( match.first.pt, match.second.pt/match.first.pt);
      }
    }
    ...
    // fill dijet efficiency and matches
    outputs[sample].dijetEfficiency[centralityBin]->Fill( matched, truthJets[0].pt);
