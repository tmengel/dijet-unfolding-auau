...
namespace PlotUtils
{
    TStyle* sphenix_style();
    void set_sphenix_style();
    void set_style();
    void draw_text(double x, double y, int color, const char* text, float size = 0.03f);

    // Compatibility wrappers for existing analysis code.
    inline void SetStyle() { set_style(); }
    inline void myText(double x, double y, int color, const char* text, float size = 0.03f)
    {
        draw_text(x, y, color, text, size);
    }
}
// 
// #include "PlotUtils.h"
