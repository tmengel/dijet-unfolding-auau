...
// Inclusive-pairing variant of createResponse_exclusive_AA.cxx: reads the
// per-leg TTree "T" written by dijet_matching_inclusive.C instead of
// dijet_matching.C. Same row shape (one row per (event, leg=1 leading/leg=2
// companion), leg 1 then leg 2 back to back, each already carrying a final
// category), but leg 2 is the leading truth jet paired with EVERY other
// truth jet in the event in turn -- (1,2), (1,3), (1,4), ... -- not just
// the immediate subleading one, so an event with N fiducial truth jets
// contributes N-1 pairs instead of at most 1. Each pair gets the same
// per-event weight (scale_factor[isample], no 1/(N-1) down-weighting),
// matching how createResponse_noempty_AA_inclusive.cxx already treats its
// own (different, C(n,2)-combinatoric) all-pairs ntuple -- this is an
// inclusive, per-companion-jet yield, not a per-event one.
//
// dijet_matching_inclusive.C's classify_leg() has no independent reco
// ranking or "remove and recalculate" peel-off (that only exists to resolve
// a mismatch against the leading+subleading dijet's own reco ranking) and
// no reco-side dphi revert -- each leg is a direct, unambiguous partition of
// (truth_in_acceptance, reco_in_acceptance) into Fill/Miss/Fake/Skip. The
// truth_ok/reco_ok reconstruction below is unchanged from
// createResponse_exclusive_AA.cxx and is exact for this simpler per-leg
// classification too -- there was never a pair-level matching-algorithm
// carve-out in that reconstruction to begin with (dijet_matching.C already
// resolved its one artifact case, kRealElsewhere, upstream of category), so
...
// closure-test unfold -- is unchanged from createResponse_exclusive_AA.cxx
// and operates purely on the resulting histograms/response, agnostic to
// where they came from. As with createResponse_noempty_AA_inclusive.cxx
// versus createResponse_noempty_AA.cxx, this writes to the SAME
// response_matrices/*.root path as createResponse_exclusive_AA.cxx for a
// given (cone_size, centrality_bin, sys_name) -- it's meant to be run
// standalone (see run_inclusive_sys_AA_exclusive.sh) to build the inclusive
// analysis's own response/unfold products, not alongside the
// leading+subleading one at the same path.
enum DijetLegCategory { kFill = 0, kMiss = 1, kFake = 2, kSkip = 3 };

int createResponse_exclusive_AA_inclusive (
	...
	const int primer = 0,
	const std::string inclusive_dir = "/home/tmengel/PPG14/rootfiles/out/inclusive"
)
{
	...
	// Two rows per (event, pair): leg 1 (truth-leading) immediately followed
	// by leg 2 (the companion truth jet for this pair) -- an event with N
	// fiducial truth jets contributes N-1 back-to-back leg pairs in a row,
	// (1,2) then (1,3) then (1,4) etc., verified when each pair is read in
	// the event loop below (the loop just steps two rows at a time
	// regardless of how many pairs a given event contributes).
	int   b_cent[3];
	float b_zvrtx[3];
	float b_sumeT[3];
	int   b_leg[3];
	...
	for (int i = 0 ; i < 3; i++)
	{
		...
		texcl[i]->SetBranchAddress("leg", &b_leg[i]);
		texcl[i]->SetBranchAddress("category", &b_category[i]);
		texcl[i]->SetBranchAddress("reco_idx", &b_reco_idx[i]);
		texcl[i]->SetBranchAddress("truth_pt", &b_truth_pt[i]);
		texcl[i]->SetBranchAddress("reco_pt", &b_reco_pt[i]);

		const Long64_t entries = texcl[i]->GetEntries();
		if ( entries % 2 != 0 )
		{
			std::cerr << "Warning: odd row count (" << entries << ") in " << path
			           << " -- expected exactly two rows (leg 1, leg 2) per pair." << std::endl;
		}
	}
	...
	// cuts), for a quick sanity check against dijet_matching.C's own
	// per-leg Fill/Miss/Fake/Skip summary.
	long n_real[3] = {0, 0, 0};
	long n_miss[3] = {0, 0, 0};
	long n_fake[3] = {0, 0, 0};
	long n_skip[3] = {0, 0, 0};
  	for (int isample = 0; isample < 3; isample++)
    {
		...
		for (Long64_t i = 0; i + 1 < entries2; i += 2)
		{
			texcl[isample] -> GetEntry(i);
			const int cat0 = b_category[isample];
			const int leg0 = b_leg[isample];
			const float truth_pt0 = b_truth_pt[isample];
			const float reco_pt0 = ( b_reco_idx[isample] >= 0 ) ? b_reco_pt[isample] : 0.0F;
			const int cent_val = b_cent[isample];
			const float zvrtx_val = b_zvrtx[isample];
			const float sumeT_val = b_sumeT[isample];

			texcl[isample] -> GetEntry(i + 1);
			const int cat1 = b_category[isample];
			const int leg1 = b_leg[isample];
			const float truth_pt1 = b_truth_pt[isample];
			const float reco_pt1 = ( b_reco_idx[isample] >= 0 ) ? b_reco_pt[isample] : 0.0F;
			...
			if ( leg0 != 1 || leg1 != 2 )
			{
				std::cerr << "Warning: expected leg 1 then leg 2 at row " << i << " of sample " << isample
				           << ", got " << leg0 << ", " << leg1 << " -- skipping this pair." << std::endl;
				continue;
			}
			...
			// Per-leg truth/reco status recovered directly from
			// dijet_matching.C's category -- no cuts are re-derived here,
			// since createResponse_noempty_AA.cxx's own bin-index-derived
			// cuts don't line up with dijet_matching.C's fixed 14/7/20/8 GeV
			// thresholds (see the file header comment).
			//   truth_ok[leg]: category is Fill or Miss (that leg's truth
			//     candidate was in dijet_matching.C's truth acceptance).
			//   reco_ok[leg]:  category is Fill or Fake (that leg has an
			//     accepted, in-range reco jet).
			// Every one of the 16 (cat0, cat1) combinations lands in exactly
			// one of skip / miss / fake / real below: both Fill -> real;
			// both truth_ok but not both reco_ok -> miss (no candidate, a
			// real match below threshold, or one outside truth fiducial);
			// both reco_ok but not both truth_ok -> fake; anything else
			// (e.g. one leg Miss, the other Fake) doesn't describe a single
			// coherent dijet candidate on either side, so it's dropped, same
			// as the PPG14 slide's "outside acceptance on both sides" case.
			const bool truth_ok0 = (cat0 == kFill || cat0 == kMiss);
			const bool truth_ok1 = (cat1 == kFill || cat1 == kMiss);
			const bool reco_ok0  = (cat0 == kFill || cat0 == kFake);
			const bool reco_ok1  = (cat1 == kFill || cat1 == kFake);

			const bool truth_good = truth_ok0 && truth_ok1;
			const bool reco_good  = reco_ok0  && reco_ok1;
			...
			const bool fake_pair  = !truth_good && reco_good;
			// Redundant given the reconstruction above (truth_good &&
			// reco_good already forces both legs to be kFill), kept for the
			// same defensive-style parity as createResponse_noempty_AA.cxx.
			const bool match_pair = (cat0 == kFill && cat1 == kFill);
			const bool real_pair  = truth_good && reco_good && match_pair;
	...
	for (int i = 0; i < 3; i++)
	{
		std::cout << "Sample " << i << " pairs -- Fill: " << n_real[i] << ", Miss: " << n_miss[i]
		          << ", Fake: " << n_fake[i] << ", Skip: " << n_skip[i] << std::endl;
	}
