...
// Redraw the aggregate data COMB plot from its cached components only.
void drawCOMBModulationDataSimple_AA(
  ...
  const std::string config = "binning_AA.config")
{
  ...
  TH1D *etaSeparated = get("h_dphi_eta_separated");
  TH1D *fittedEta = get("h_dphi_eta_separated_fit");
  TH1D *background = get("h_flow_background_nominal");
  TH1D *nominal = get("h_dphi_nominal_subtracted");
  TH1D *down = get("h_dphi_COMBDown_subtracted");
  TH1D *up = get("h_dphi_COMBUp_subtracted");
  TH1D *signalRegion = get("h_signal_region");
  TH1D *normalizationRegion = get("h_flow_normalization_region");
  if (!raw || !etaSeparated || !fittedEta || !background || !nominal ||
      !down || !up || !signalRegion || !normalizationRegion) return;
  ...
  fittedEta->SetLineColor(kOrange + 7);
  fittedEta->SetLineWidth(3);
  ...
  TCanvas canvas("c_dphi_data_comb_simple", "c_dphi_data_comb_simple", 650, 560);
  ...
  legend.AddEntry(raw, "Raw #Delta#phi pairs", "l");
  legend.AddEntry(background, "Nominal flow modulation", "l");
  legend.AddEntry(etaSeparated, "#Delta#eta-separated fit input", "l");
  legend.AddEntry(fittedEta, "Fitted #Delta#eta histogram", "l");
  ...
  std::cout << "Redrew " << stem << ".pdf from cached data components"
            << std::endl;
}
