...
root -l -b -q "createResponse_noempty_AA.cxx+(\"${AUAU_CONFIG}\",2,10,${cone_size},${centrality},0)"

echo "Building reproducible half-sample closure response for centrality ${centrality}"
root -l -b -q "createResponse_noempty_AA.cxx+(\"${AUAU_CONFIG}\",1,10,${cone_size},${centrality},0)"

echo "Drawing full and half closure tests for centrality ${centrality}"
root -l -b -q "drawClosureTests_AA.C(${cone_size},${centrality},1,\"${AUAU_CONFIG}\")"
