...
)
{
	...
	for (int i = 0 ; i < 3; i++)
	{
		...
		if ( tn[i]->GetBranch("psi2") != nullptr ) tn[i]->SetBranchAddress("psi2", &psi2[i]);
		if ( tn[i]->GetBranch("psi3") != nullptr ) tn[i]->SetBranchAddress("psi3", &psi3[i]);
		if ( tn[i]->GetBranch("sublead_flag") != nullptr ) tn[i]->SetBranchAddress("sublead_flag", &sublead_flag[i]);
		if ( tn[i]->GetBranch("maxpttruth_r04") != nullptr ) tn[i]->SetBranchAddress("maxpttruth_r04", &maxpttruthr04[i]);

		bool is_new_file_schema = tn[i]->GetBranch("psi2") != nullptr;
	...
	std::cout << "n_dijets: " << n_dijets[0] << " " << n_dijets[1] << " " << n_dijets[2] << std::endl;	
	...
	int n_fills[3] = {0, 0, 0};
	int n_miss_from_acc[3] = {0, 0, 0};
	int n_miss_from_match[3] = {0, 0, 0};
