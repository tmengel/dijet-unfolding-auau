...
)
{
	...
	int read_nbins = rb.get_nbins();
	int primer = rb.get_primer();
  	...
  	for (int i = 0; i < total_entries; i++)
    {
    	...
    	if (i % int(total_entries/10) == 0) std::cout << "Event: " << i << " / " << total_entries << "\r" << std::flush;
      	...
      	for (int j = 0; j < nbins; j++)
		{
			...
			else
			{
				...
				else
				{
					...
					par1 = std::isfinite(dphi_zyam_integral)? dphi_zyam_integral : 0.0F;
				}
				...
				h_pt1_pt2_ratio->SetBinContent(gbin, dphi_signal_integral > 0 ? sig/dphi_signal_integral : 0.0F);
			}
		}
	}
