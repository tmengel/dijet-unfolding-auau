...
)
{
  ...
  const std::vector<std::string> draw_as_lines = {
    "data_background", "data_background_COMBDown", "data_background_COMBUp","data_fit"
  ...
  std::cout << "Data v22/v33 = " << v22_data[0]/v33_data[0] << std::endl;
  std::cout << "Sim v22/v33 = " << v22_sim[0]/v33_sim[0] << std::endl;  
  ...
  displayed.front().histogram->SetMaximum(1.35*maximum);
  ...
  for (std::size_t index = 0; index < displayed.size(); ++index)
  {
    ...
    if (isSimulation[index])
      displayed[index].histogram->Draw(index == 0 ? "hist" : "hist same");
    // check if its a background curve and draw as line
    else if (draw_as_lines.end() != std::find(draw_as_lines.begin(), draw_as_lines.end(), definitions[index].selection))
      displayed[index].histogram->Draw(index == 0 ? "hist" : "hist same");
    else
      displayed[index].histogram->Draw(index == 0 ? "p" : "p same");
    // draw sim as hist and data as points
    // if (displayed[index].histogram->GetLineStyle() == 1)
    //   displayed[index].histogram->Draw(index == 0 ? "hist" : "hist same");
    // else
    //   displayed[index].histogram->Draw(index == 0 ? "p" : "p same");
    // displayed[index].histogram->Draw(index == 0 ? "hist" : "hist same");

    // draw regions as filled boxes
    ...
    // legend.AddEntry(curve.histogram.get(), curve.label.c_str(), "l");
  {
    if (isregion[&curve - &displayed[0]])
      continue;
      // legend.AddEntry(curve.histogram.get(), curve.label.c_str(), "f");
    else if (isSimulation[&curve - &displayed[0]])
      legend.AddEntry(curve.histogram.get(), curve.label.c_str(), "l");
    // else
    else if (draw_as_lines.end() != std::find(draw_as_lines.begin(), draw_as_lines.end(), definitions[&curve - &displayed[0]].selection))
      legend.AddEntry(curve.histogram.get(), curve.label.c_str(), "l");
    else
  ...
  dlutility::drawText(Form("Sim v_{2,2} = %.3f , v_{3,3} = %.3f", 2*v22_sim[0], 2*v33_sim[0]), 0.18, 0.62, 0, kBlack, 0.028);
  // dlutility::drawText(Form("
  // add text for v22 and v33
  // dlutility::drawText(Form("Data v_{2,2} = %.3f #pm %.3f", v22_data[0], v22_data_err[0]), 0.18, 0.66, 0, kBlack, 0.028);
  // dlutility::drawText(Form("Data v_{3,3} = %.3f #pm %.3f", v33_data[0], v33_data_err[0]), 0.18, 0.62, 0, kBlack, 0.028);
  // dlutility::drawText(Form("Sim v_{2,2} = %.3f #pm %.3f", v22_sim[0], v22_sim_err[0]), 0.18, 0.58, 0, kBlack, 0.028);
  // dlutility::drawText(Form("Sim v_{3,3} = %.3f #pm %.3f", v33_sim[0], v33_sim_err[0]), 0.18, 0.54, 0, kBlack, 0.028);

    dlutility::drawText(Form("Data v_{2,2} = %.3f #pm %.3f", v22_data[0], 0.3*v22_data[0]), 0.58, 0.62, 0, kBlack, 0.028);
    dlutility::drawText(Form("Data v_{3,3} = %.3f #pm %.3f", v33_data[0], 0.3*v33_data[0]), 0.58, 0.58, 0, kBlack, 0.028);
    dlutility::drawText(Form("Sim v_{2,2} = %.3f #pm %.3f", 2*v22_sim[0], 0.3*v22_sim[0]), 0.58, 0.54, 0, kBlack, 0.028);
    dlutility::drawText(Form("Sim v_{3,3} = %.3f #pm %.3f", 2*v33_sim[0], 0.3*v33_sim[0]), 0.58, 0.48, 0, kBlack, 0.028);
